...
class rgb_only_Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False):
        super().__init__()
        self.netG_A = dsuGeneratorRGB2NIR_direct(input_nc, output_nc, 'spectralinstance', ngf, norm_layer=norm_layer,
                                          use_dropout=use_dropout)
    
    def forward(self, nir_img):
        # Direct RGB prediction without HSV guidance
        fake_B = self.netG_A(nir_img)
        return fake_B
