...
def get_data_paths():
    ...
    nir_files_png = glob.glob(os.path.join(path_train_nir, '*.png'))
    nir_files_tiff = sorted(glob.glob(os.path.join(path_train_nir, '*.tiff')))
    # nir_files_jpg = sorted(glob.glob(os.path.join(path_train_nir, '*.jpg')))

    nir_files = sorted(nir_files_tiff + nir_files_png)
    nir_files.sort()
    nir_files_png_val = glob.glob(os.path.join(path_val, 'nir', '*.png'))
    nir_files_tiff_val = glob.glob(os.path.join(path_val, 'nir', '*.tiff'))
    nir_files_val = sorted(nir_files_tiff_val + nir_files_png_val)
    nir_files_val.sort()
    # rgb_files = glob.glob(os.path.join(path_train_rgb, '*.png'))
    rgb_files_png = sorted(glob.glob(os.path.join(path_train_rgb, '*.png')))
    # rgb_files_tiff = sorted(glob.glob(os.path.join(path_train_rgb, '*.tiff')))
    rgb_files_jpg = sorted(glob.glob(os.path.join(path_train_rgb, '*.jpg')))

    rgb_files = sorted(rgb_files_png + rgb_files_jpg)
    rgb_files.sort()
    rgb_files_png_val = glob.glob(os.path.join(path_val, 'rgb', '*_rgb_reg.png'))
    # rgb_files_tiff_val = sorted(glob.glob(os.path.join(path_val,'rgb', '*.tiff')))
    rgb_files_jpg_val = sorted(glob.glob(os.path.join(path_val, 'rgb','*.jpg')))
    rgb_files_val = sorted(rgb_files_png_val + rgb_files_jpg_val)
    rgb_files_val.sort()
